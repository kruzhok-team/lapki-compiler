...
void Keypad::addEventListener(void* context, void (*listener)(void*, char)){
...
void Keypad::transitionTo(byte idx, KeyState nextState) {
	...
	if (single_key)  {
	  	if ( (keypadEventListener!=NULL) && (idx==0) )  {
			keypadEventListener(eventListenerContext, key[0].kchar);
		}
	}
	...
	// Calls keypadEventListener on any key that changes state.
	  	...
	  	if (keypadEventListener!=NULL)  {
			keypadEventListener(eventListenerContext, key[idx].kchar);
		}
	}
}
