...
__attribute__((section(".usbbuf"))) volatile static USB_BTABLE_ENTRY BTable[8] = {0};

__attribute__((aligned(2)))
__attribute__((section(".usbbuf"))) volatile static char EP0_Buf[2][64] = {0};
...
static void USB_HandleControl()
{
    ...
    { // Получили сообщение в endpoint 0
        ...
        { // Это SETUP пакет
            ...
            if ((setup->RequestType & DEFINE_bmRequestType_Dir_Mask) == DEFINE_bmRequestType_Dir_HostToDevice && setup->Length > 0)
            {
...
uint8_t guidRegistry[142] =
    {
        ...
        ,
        0x44, 0x00, 0x65, 0x00, 0x76, 0x00, 0x69, 0x00, 0x63, 0x00, 0x65, 0x00, 0x49, 0x00, 0x6e, 0x00, 0x74, 0x00, 0x65, 0x00, 0x72, 0x00, 0x66, 0x00, 0x61, 0x00, 0x63, 0x00, 0x65, 0x00, 0x47, 0x00, 0x55, 0x00, 0x49, 0x00, 0x44, 0x00, 0x00, 0x00 //	NUL-terminated Unicode String (LE)	Property name "DeviceInterfaceGUID"

        ,
        0x4e, 0x00, 0x00, 0x00 //	DWORD (LE)	Property data length (78 bytes)

        ,
        0x7b, 0x00, 0x30, 0x00, 0x43, 0x00, 0x30, 0x00, 0x34, 0x00, 0x30, 0x00, 0x30, 0x00, 0x43, 0x00, 0x39, 0x00, 0x2d, 0x00, 0x45, 0x00, 0x33, 0x00, 0x35, 0x00, 0x34, 0x00, 0x2d, 0x00, 0x34, 0x00, 0x33, 0x00, 0x31, 0x00, 0x30, 0x00, 0x2d, 0x00, 0x41, 0x00, 0x45, 0x00, 0x44, 0x00, 0x35, 0x00, 0x2d, 0x00, 0x45, 0x00, 0x46, 0x00, 0x45, 0x00, 0x38, 0x00, 0x43, 0x00, 0x39, 0x00, 0x46, 0x00, 0x41, 0x00, 0x38, 0x00, 0x33, 0x00, 0x30, 0x00, 0x38, 0x00, 0x7d, 0x00, 0x00, 0x00 // NUL-terminated Unicode String (LE) Property name "{0c0400c9-e354-4310-aed5-efe8c9fa8308}"
};

static void
USB_HandleSetup(USB_SETUP_PACKET *setup)
{
    USB_CopyMemory((volatile uint16_t *)setup, (volatile uint16_t *)&ControlState.Setup, sizeof(USB_SETUP_PACKET));
    ControlState.Transfer.Length = 0;
    ...
    */
    ...
    else if ((setup->RequestType & 0x60) == 0)
    {
        ...
        else if ((setup->RequestType & 0x0F) == 0x01)
        { // Interface requests
            switch (setup->Request)
            {
            case 0x00: // Get Status
        ...
        else if ((setup->RequestType & 0x0F) == 0x02)
        { // Endpoint requests
            switch (setup->Request)
            {
            case 0x00: // Get Status
...
static void
USB_PrepareTransfer(USB_TRANSFER_STATE *transfer, short *ep, char *txBuffer, short *txBufferCount, short txBufferSize)
{
...
static void
USB_CopyMemory(volatile uint16_t *source, volatile uint16_t *target, uint16_t length)
{
