...
extern "C"
void SysHalt(const char *reason);
...
#define CORTEX_MAXIMUM_PRIORITY     0U // The maximum allowed priority level is always zero.
#define CORTEX_PRIORITY_SVCALL      (CORTEX_MAXIMUM_PRIORITY + 1U) // CORTEX_MAXIMUM_PRIORITY+1 reserves the CORTEX_MAXIMUM_PRIORITY priority level as fast interrupts priority level
#define CORTEX_PRIORITY_PENDSV      (CORTEX_PRIORITY_SVCALL + 1U) // highest priority that cannot preempt the kernel
...
#if SYS_DBG_CHECK
    void Check(bool IsOk, const char *reason) { if(!IsOk) SysHalt(reason); }

    // Verifies that the system is in an appropriate state for invoking an I-class API function
    void CheckClassI() { if((isr_cnt  < 0L) or (lock_cnt <= 0L)) SysHalt("#BadStateI"); }
    void CheckClassS() { if((isr_cnt != 0L) or (lock_cnt <= 0L)) SysHalt("#BadStateS"); }
    void CheckLock() {
...
void VtDoTickI() {
    ...
    if(delta < (systime_t)SYS_ST_TIMEDELTA) delta = (systime_t)SYS_ST_TIMEDELTA;
...
void VirtualTimer::DoSetI(systime_t delay, vtfunc_t vtfunc, void *APtr) {
    ...
    if(&vtlist == (VirtualTimersList_t*)vtlist.next) {
        // The delta list is empty => current time becomes the new delta list base time, the timer is inserted
        vtlist.lasttime = now;
        vtlist.next = this;
        vtlist.prev = this;
    ...
    if(Fdelta < TimeDiff(vtlist.lasttime, now)) {
        ...
        // A small delay that will become the first element in the delta list and next deadline
        deadline_delta = Fdelta;
        SysTimer::SetAlarm(TimeAdd(vtlist.lasttime, deadline_delta));
    }
    // The delta list is scanned in order to find the correct position for this timer
    ...
    p->delta -= Fdelta;
    // Special case when the timer is in last position in the list, the value in the header must be restored
...
void VirtualTimer::DoResetI() {
    ...
    pCallback = nullptr;
    // If the timer is not the first of the list then it is simply unlinked, else the operation is more complex
    if(vtlist.next != this) {
    ...
    // Make sure to not schedule an event closer than SYS_ST_TIMEDELTA ticks from now
    if(Fdelta < (systime_t)SYS_ST_TIMEDELTA) Fdelta = nowdelta + (systime_t)SYS_ST_TIMEDELTA;
    else Fdelta = nowdelta + Fdelta;
...
void SchDoRescheduleI() {
    ...
    currp = rlist.queue.FifoRemove();
    currp->state = ThdState::Current;
    // If the round-robin mechanism is disabled then the thread goes always ahead of its peers
    oldtp = SchReadyAheadI(oldtp);
    SwitchContext(currp, oldtp); // Finally, switch context
}
 ...
 *
 * @param[in] timeout   the number of ticks before the operation timeouts, the following special values are allowed:
 *                      - @a TIME_IMMEDIATE immediate timeout.
 *                      - @a TIME_INFINITE no timeout.
 *                      .
 * @return              A message specifying how the invoking thread has been released from the semaphore.
 * @retval Ok           if the thread has not stopped on the semaphore or the semaphore has been signaled.
 * @retval Reset        if the semaphore has been reset using @p Semaphore.Reset().
 * @retval Timeout      if the semaphore has not been signaled or reset within the specified timeout.
 *
 * @sclass
 */
...
/* This thread has the lowest priority in the system, its role is only to
   ...
   Stack size set to 16 because the idle thread does have a stack frame when compiling without optimizations. You may
...
namespace Sys { // =========================== Sys =============================
    ...
    volatile uint32_t Time_ = SYS_TIM->CNT;
...
void Init() {
    ...
    // Stack check init. Set up the base address of the static main thread stack, the symbol must be provided externally
...
Thread *CreateThd(void *Workspace, size_t Sz, uint32_t Prio, tfunc_t ThdFunc) {
    dbg.Check((Workspace != NULL) &&
               ...
               MEM_IS_ALIGNED(Sz, STACK_ALIGN) &&
               (Prio <= HIGHPRIO), "Bad CreateThd params");
    Lock();
...
void DbgAssert(bool IsOk, const char* MsgIfNotOk) { dbg.Check(IsOk, MsgIfNotOk); }
...
void _port_irq_epilogue() {
    ...
    if(SCB->ICSR & SCB_ICSR_RETTOBASE_Msk) {
        McuExtCtx_t *ctxp;
#if CORTEX_USE_FPU == TRUE // Enforcing a lazy FPU state save by accessing the FPCSR register
        (void)__get_FPSCR();
#endif
        ctxp = (McuExtCtx_t*)__get_PSP();
        // Adding an artificial exception return context, there is no need to populate it fully
        ctxp--;
        ...
        // The exit sequence is different depending on if a preemption is required or not
        if(SchIsPreemptionRequired()) { // Preemption is required => we need to enforce a context switch
            ctxp->pc = (regarm_t)_port_switch_from_isr;
        }
        else { // Preemption not required, we just need to exit the exception atomically
...
void SVC_Handler() {
    ...
    ctxp = (McuExtCtx_t*)__get_PSP();
    // Discard the current exception context and position the stack to point to the real one
    ctxp++;
...
// PendSV vector used for exception mode re-entering after a context switch in compact kernel mode.
extern "C"
void PendSV_Handler() {
    McuExtCtx_t *ctxp;
#if CORTEX_USE_FPU
...
inline void Init() {
    RCU->EnTimer(SYS_TIM);
    DBGMCU->StopTmrInDbg(SYS_TIM); // En the stop mode during debug for this timer
